...
export default function App() {
  ...
  async function sendPrompt(draft?: ComposerDraft) {
    ...
    if (content.toLowerCase() === "test" || content.toLowerCase() === "/test") {
      setPrompt("");
      const userMsg = {
        info: {
          role: "user",
      ...
      const botMsg = {
        ...
        parts: [{ type: "text", text: "Test successful! The chat interface is working correctly." }]
      };

      setMessages([...messages(), userMsg as any, botMsg as any]);
      return;
    }
    ...
    if (!c) {
      ...
      const botMsg = {
        ...
        parts: [{ type: "text", text: "MAYA is disconnected. This is a mock response from the Test chat page.\n\nYou said: " + content }]
      };
  ...
  async function createSessionAndOpen() {
    const c = client();
    if (!c) {
      // Mock session for Test chat when backend is disconnected
      ...
      setSessions([mockSession, ...currentStoreSessions]);
      ...
      if (wsId) {
        ...
        setSidebarSessionsByWorkspaceId((prev) => ({
          ...prev,
          [wsId]: [{ id: mockId, title: "Test chat", slug: "test-chat", time: mockSession.time, directory: mockSession.directory }, ...currentSidebarSessions],
        }));
      }

      await selectSession(mockId);
      goToSession(mockId);
      return mockId;
    }
