...
// MAYA orchestrator alias paths (desktop expects a "maya-orchestrator" sidecar)
const mayaOrchestratorBaseName = "maya-orchestrator";
const mayaOrchestratorName = process.platform === "win32" ? `${mayaOrchestratorBaseName}.exe` : mayaOrchestratorBaseName;
const mayaOrchestratorPath = join(sidecarDir, mayaOrchestratorName);
const mayaOrchestratorTargetName = orchestratorTargetTriple
  ? `${mayaOrchestratorBaseName}-${orchestratorTargetTriple}${orchestratorTargetTriple.includes("windows") ? ".exe" : ""}`
  : null;
const mayaOrchestratorTargetPath = mayaOrchestratorTargetName ? join(sidecarDir, mayaOrchestratorTargetName) : null;
...
if (existsSync(orchestratorBuildPath)) {
  ...
  // Copy to maya-orchestrator alias
  if (mayaOrchestratorTargetPath) {
    const shouldCopyTarget =
      didBuildOrchestrator ||
      !existsSync(mayaOrchestratorTargetPath) ||
      isStubBinary(mayaOrchestratorTargetPath);
    if (shouldCopyTarget && orchestratorBuildPath !== mayaOrchestratorTargetPath) {
      try {
        if (existsSync(mayaOrchestratorTargetPath)) unlinkSync(mayaOrchestratorTargetPath);
      } catch {
        // ignore
      }
      copyFileSync(orchestratorBuildPath, mayaOrchestratorTargetPath);
    }
  }
}
