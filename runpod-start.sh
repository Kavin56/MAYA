...
if [ -f "$(dirname "$0")/src/owl-backend/.env" ]; then
  ...
  echo "[warn] No .env found at src/owl-backend/.env"
fi
...
fuser -k 8787/tcp 4096/tcp 5000/tcp 2>/dev/null || true
sleep 2
echo "      Done."
...
# NOTE: Single line to avoid any shell line-continuation issues
nohup pnpm --filter maya-server dev --host 0.0.0.0 --port "$SERVER_PORT" --approval auto --cors "*" --opencode-base-url "http://127.0.0.1:$OPENCODE_PORT" --workspace "$MAYA_WORKSPACES" > /tmp/maya-server.log 2>&1 &
SERVER_PID=$!
...
nohup ngrok http --domain="$NGROK_DOMAIN" "$SERVER_PORT" > /tmp/ngrok.log 2>&1 &
NGROK_PID=$!
...
echo "║  Ping test: https://$NGROK_DOMAIN/ping"
echo "╟───────────────────────────────────────────────╢"
echo "║  Logs:                                        ║"
